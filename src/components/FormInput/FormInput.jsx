...
export const FormInput = React.memo((props) => {
  ...
  if (props.emailPattern) {
    return (
      <label htmlFor={props.type} className="form__label">
        {props.title}
        <input value={props.value} onChange={props.handleChange} type={props.type} className="form__input" name={props.type} id={props.type} minLength={`${props.minL}` || ''} maxLength={`${props.maxL}` || ''} required pattern={props.emailPattern} />
