...
export const RegisterForm = React.memo((props) => {
  ...
  return (
    <form onSubmit={handleSubmit} className={`form form_type_auth`} name='register-form' noValidate>
      ...
      <label htmlFor="name" className="form__label form__label_type_auth">
      Имя
        <input onChange={handleChange} type="name" className={`form__input form__input_type_name form__input_type_auth ${isFormValid ? '' : 'form__input_type_error'}`} name="name" id="name" minLength="2" maxLength="40" required />
      ...
      <button disabled={!isFormValid} type="submit" className={`form__item form__button form__button_type_register`} value={props.buttonText} aria-label="Кнопка отправки формы">{`${props.isLoading ? 'Ждемс...' : props.buttonText}`}</button>
    </form>
  )
})